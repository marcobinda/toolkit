...
// Any varible of this type will have access to all the methos with the reciever *Tools
type Tools struct {
	MaxFileSize      int
	AllowedFileTypes []string
	MaxJSONSize      int
	AllowUnkownFiels bool
}
...
// UploadedFile is a struct used to save ifnormation about an uploaded file
...
// UploadOneFile is just a convenience metho that calls UploadFiles, but expects only one file to be uploaded.
...
// It returns a slice cotaining the newly named files, the original file names, the size of the file
...
func (t *Tools) UploadFiles(r *http.Request, uploadDir string, rename ...bool) ([]*UploadedFile, error) {
	...
	err := t.CreatDirIfNotExists(uploadDir)
	if err != nil {
		return nil, err
	}
	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				buff := make([]byte, 512) // look at the first 512 bytes of the file to determine mime type
				_, err = infile.Read(buff)
				if err != nil {
					return nil, err
				}

				// check to see if the file type is permited
				allowed := false
				...
				if !allowed {
					return nil, errors.New("the uploaded file type is no permitted")
				}
...
// CreatDirIfNotExist craetes a directory, and all necessary parents, if it does not exists
func (t *Tools) CreatDirIfNotExists(path string) error {
	const mode = 0755
...
// by setting content-disposition. It also allows especification of the display name
func (t *Tools) DownloadStaticFile(w http.ResponseWriter, r *http.Request, p, file, displayName string) {
	fp := path.Join(p, file)
	w.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=\"%s\"", displayName))
	// w.Header().Add("Content-Disposition", "attachment; filename="+displayName)
...
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data interface{}) error {
	...
	if err != io.EOF {
		return errors.New("body mus contain only one JSON value")
	}

	return nil
}

// WriteJSON takes a response satus code and arbitrary data and writes json to the client
...
// ErrorJSON takes an error, and optionally a status code, and generartes and send a JSON error message
...
func (t *Tools) PushJSONToRemote(uri string, data interface{}, client ...*http.Client) (*http.Response, int, error) {
	...
	// build request and se the header
