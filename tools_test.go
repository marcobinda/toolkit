...
func TestTools_CreatDirIfNotExists(t *testing.T) {
	var testTool Tools

	err := testTool.CreatDirIfNotExists("./testdata/myDir")
	if err != nil {
		t.Error(err)
	}
...
func TestTools_Slugify(t *testing.T) {
	...
	for _, e := range slugTests {
		...
		if !e.errorExpected && slug != e.expected {
			t.Errorf("%s: wrong slug returned; expected %s but gt %s", e.name, e.expected, slug)
		}
	}
}
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		testTool.AllowUnkownFiels = e.allowUnknown
...
func TestTools_WriteJSON(t *testing.T) {
	...
	if err != nil {
		t.Errorf("faile to write JSON: %v", err)
	}
